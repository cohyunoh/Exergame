...
for(i=0;i<segments;i++){
	...
	draw_text_transformed_color(x+cos(degtorad(rotate+angles[i]+(angle/2)))*(radius/2+30),y-sin(degtorad(rotate+angles[i]+(angle/2)))*(radius/2+30),string(segments-i),4.5,4.5,rotate+angles[i]+(angle/2-90),c_black,c_black,c_black,c_black,1);
}
...
if(reverse){
	if(balance_obj_ticker.image_angle>=180){
		is_spinning=false;
		reverse=false;
